...
class Start
  ...
  API_KEY = 'WrtMZqY9RNwFzesQArWnyWjuQ6Gwdezp'
  GLOBAL_TIMEOUT = 90 #900 seconds div 10

  def self.check
    @gt = GLOBAL_TIMEOUT

    @message_body = String.new
    verifications = %w(WebsiteEvent::Safebrowsing WebsiteEvent::Safebrowsing WebsiteEvent::MobileRedirect WebsiteEvent::LinkToBlacklist WebsiteEvent::StaticLinkToBlacklist WebsiteEvent::DynamicLinkToBlacklist WebsiteEvent::AVBlacklist)
    ...
    uri = URI('https://sitesecure.ru//api/mail_ru/scan.json')
    ...
    request.set_form_data({domain: expected['url'], api_key: API_KEY})
    ...
    uri = URI('https://sitesecure.ru//api/mail_ru/result.json')
    ...
    request = Net::HTTP::Get.new(uri)
    request.set_form_data({uid: obj.uid, api_key: API_KEY})
    ...
    end while obj.status == 'progress'

    # Loop global timeout
    # End loop if data not empty or timeout exceeds
    begin
      data = obj.data
      @gt = @gt - 1
      sleep(10)
    end while data.empty? && @gt <= 0
    ...
    @message_body << validate_item(data, 'WebsiteEvent::AVBlacklist', to_boolean(expected['WebsiteEvent::AVBlacklist']))
  ...
  def self.validate_item(data, source, expected_value)
    ...
    if data.each_index.select { |i| data[i].source == source } == []
      ...
      if data[data.each_index.select { |i| data[i].source == source }[0]].vulnerable? == expected_value
        ...
        @result << to_string(data[data.each_index.select { |i| data[i].source == 'WebsiteEvent::AVBlacklist' }[0]].vulnerable?)
      end
    end
